...
function PolyCoeff = InitialCoeff_DesiredTrajectories_x_pruebas(robot,gait_parameters,x0cyc,xfcyc,xmid,xp0cyc)
...
PosD = [x0cyc,IniPos(2);
        xfcyc, hT_d(2)];
VelD = [x0cyc, IniVel(2)/xp0cyc;
        xfcyc, hpT_d(2)];    
PolyCoeff.hd2 = findPolyCoeff(PosD,VelD,[]); % posd,veld,accd
%
PosD = [x0cyc,IniPos(3);
        xfcyc, hT_d(3)];
VelD = [x0cyc, IniVel(3)/xp0cyc;
        xfcyc, hpT_d(3)];    
