...
function [t,X0,Xt] = robot_step_EssModel_tJerk(X_final)
...
Rollp0 = qfp0(11); Pitchp0 = qfp0(12);
% ====================================================================
% -------------------------------------------------------------
...
ZeroDynamics_impactTime_test(robot.CoM,qfp0,robot,gait_parameters);
% ---------------------------------------------------------------------------------
...
X0 = [robot.CoM(1:2); robot.q(13:20); Hips(1:2)';
    xp0; yp0; q13p0; q14p0; q15p0; q16p0; q17p0; q18p0; q19p0; q20p0; Rollp0; Pitchp0];
% % -----------------------------------------------------------------------------------------------
% % ----------------------------------------------------------------------------------------------------
% [t,Xt] = ode45(@dynam_HZDtimeLipReference,0:1e-3:T,X0);
% [Xt] = ode4(@dynam_HZDtimeLipReferenceOnlineJerk,0:1e-3:T,X0);
% t = 0:1e-3:T;
[Xt] = ode4(@dynam_HZDtimeLipReferenceOnlineJerk,0:1e-2:T,X0);
t = 0:1e-2:T;
...
if isempty(OutOfWorkSpace) % If the CoM of the robot is always inside the workspace of the robot....
    qf_final = Xt(end,1:12)'; % CoM position at the end of the step
    qfp_final = Xt(end,13:24)'; % CoM velocity at the end of the step
