...
function [ XD ] = dynam_HZDtimeLipReferenceOnlineJerkDS(t,X)
%%Simple support dynamic model (zero dynamics) for the robot model
%   State variable (x, y, xp, yp)
global robot gait_parameters ZMPRef Xref
% % init;
x = X(1);
y = X(2);
xp = X(13);
...
if isempty(OutOfWorkSpace)
    [qfpp, ~, ~, ~, ~, ~, ~] = Desired_qfpp_HZDtimeQPLipReferenceDS(Xref,ZMPd,robot,[qf;qfp],gait_parameters,t);
else
    qfpp = zeros(12,1);
