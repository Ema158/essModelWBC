...
function [ XD ] = dynam_HZDtimeLipReferenceOnlineJerkNoWBCDS(t,X)
%%Simple support dynamic model (zero dynamics) for the robot model
%   State variable (x, y, xp, yp)
global robot gait_parameters xpp
% % init;
x = X(1);
y = X(2);
xp = X(3);
...
k = evaluateTref(robot.tRef,t+gait_parameters.T*(4)); %Porque la fase de SS vale 4 veces la de DS
if isempty(xpp)
    xpp(1) = 0;
    xpp(2) = 0;
end
[Xref,ZMPRef] = MpcLIPOnlineJerkConstraintsDS(gait_parameters,[x;y;xp;yp;xpp(1);xpp(2)],robot.Zref,robot.ZMax,robot.ZMin,k,robot.Px,robot.Pu);
...
if isempty(OutOfWorkSpace)
    [qfpp, ~, ~, ~, ~, ~, ~] = Desired_qfpp_HZDtimeQPLipReferenceNoWBC(Xref,ZMPd,robot,[qf;qfp],gait_parameters,t);
else
    qfpp = zeros(12,1);
