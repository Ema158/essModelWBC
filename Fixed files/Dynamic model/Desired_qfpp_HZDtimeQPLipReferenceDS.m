function [qfpp, JPhi, JPhipPhip, NE1, NE2, NE3, qfp] = Desired_qfpp_HZDtimeQPLipReferenceDS(xRef,ZMPd,robot_i,X,gait_param_i,t)
...
% Aceleración xpp, ypp, zpp 
Q = eye(12,12);
Q(1,1) = 0;
Q(2,2) = 0;
%
Q(3:10,3:10) = 0.05*eye(8,8); %Walking
Q(11,11) = 0.05; %Walking
Q(12,12) = 0.05; %Walking
% %
% Q(3:10,3:10) = 0.00001*eye(8,8); %0.00001
% Q(11,11) = 0.002; %0.002
% Q(12,12) = 0.002; %0.002 Standing
%
% Q(3:10,3:10) = 1000000*eye(8,8); %Walking
% Q(11,11) = 1000000; %Walking
% Q(12,12) = 1000000; %Walking
%
A = DrBar;
b = -HrBar;
...
%
%
% varThetaRef = varThetaPdControl(robot,xRef);
k = evaluateTref(robot.tRef,t+gait_parameters.T*(4));
varThetaRef = varThetaPdControlZmpTerm(robot,xRef,[robot.Zref(1,k);robot.Zref(2,k)]);
p = -varThetaRef'*Q;
%
alpha=0.15; %0.01 0.001 0.025
Qnew = Q + alpha/2*(DrBar'*DrBar);
pnew = -varThetaRef'*Q + alpha*(HrBar'*DrBar);
%
% Qnew = Q'*Q + alpha*(DrBar'*DrBar);
% pnew = -2*varThetaRef'*(Q'*Q) + 2*alpha*(HrBar'*DrBar);
qfpp = quadprog(Q,p,[],[],A,b,[],[],[],options); %seg zmp perfecto
% qfpp(12) = -qfpp(12); %
% if t>=0&&t<=0.05
%     qfpp(2) = qfpp(2) + 0.5;
% end
% qfpp = quadprog(Qnew,pnew,[],[],[],[],[],[],[],options); %error en seg
% qfpp = -DrBar\HrBar;
