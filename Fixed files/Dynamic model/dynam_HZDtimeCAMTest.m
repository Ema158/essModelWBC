...
function [ XD ] = dynam_HZDtimeCAMTest(t,X)
...
[Xref,ZMPRef] = MpcLIPOnlineAccConstraintsV2(gait_parameters,[x;y;xp;yp],robot.Zref,k,robot.Px,robot.Pu);
% ZMPd = [ZMPRef(1);ZMPRef(2)];
ZMPd = [0;0.05];
% CAMd = [0;0.1];
CAMd = [0,polyval(gait_parameters.CAMyCoeff,t)];
% XrefEval = [0,0];
% ZMPd = [0;0.05];
...
if isempty(OutOfWorkSpace)
    [qfpp, ~, ~, ~, ~, ~, ~] = Desired_qfpp_HZDtimeCAMTestV2(Xref,ZMPd,CAMd,robot,[qf;qfp],gait_parameters,t);
else
    qfpp = zeros(12,1);
