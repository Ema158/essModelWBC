...
function [t,X0,Xt,t_SS,Xt_SS,t_DS,Xt_DS] = robot_TwoDiffSteps_SS_DS_EssModel_t(X_final,gait_parameters_SS_CurrentStep,gait_parameters_DS_CurrentStep,gait_parameters_DS_PreviousStep)
...
T = gait_parameters_SS_CurrentStep.T;
...
[q_end,qp_end,~,~,~] = JointsPosVel_from_CoMPosVel_HZDtime(qf_final,qfp_final,robot,gait_parameters_DS_PreviousStep,T);
...
[qfp0,robot] = impact_Pos_Vel(robot,gait_parameters_SS_CurrentStep.v_foot_f); %
...
gait_parameters_SS_CurrentStep.transition = true;
PolyCoeff = Coeff_DesiredTrajectories_t_ver2(robot,gait_parameters_SS_CurrentStep);
gait_parameters_SS_CurrentStep.PolyCoeff = PolyCoeff;   
gait_parameters = gait_parameters_SS_CurrentStep;

% Just to chck if the all the controlled variables are in the ZERO DYNAMICS MANIFOLD
ZeroDynamics_impactTime_test(x0,y0,qfp0,robot,gait_parameters_SS_CurrentStep);
% ---------------------------------------------------------------------------------
...
[q_end,qp_end,~,~,~] = JointsPosVel_from_CoMPosVel_HZDtime(qf_final,qfp_final,robot,gait_parameters_SS_CurrentStep,T);
% By using final joints information, the structure of the robot is updated, i.e Jacobians,transformation matrices, etc.
robot = robot_move(robot,q_end);
robot.qD = qp_end;
%% DS phase
...
disp('DS phase')
disp('--------------------------')
T = gait_parameters_DS_CurrentStep.T;
...
% COMPUTING POLYNOMIALS
% Computing of Coefficients for the polinomials by taking into account the states after impact
gait_parameters_DS_CurrentStep.transition = false; %Param21 y 45d 45e
%  gait_parameters_DS.transition = true;
PolyCoeff = Coeff_DesiredTrajectories_t_ver2(robot,gait_parameters_DS_CurrentStep);
gait_parameters_DS_CurrentStep.PolyCoeff = PolyCoeff;   
gait_parameters = gait_parameters_DS_CurrentStep;
%
...
if isempty(OutOfWorkSpace) % If the CoM of the robot is always inside the workspace of the robot....
    ...
    [q,qp,~,~,~] = JointsPosVel_from_CoMPosVel_HZDtime(qf_final,qfp_final,robot,gait_parameters_DS_CurrentStep,t_DS(end));
