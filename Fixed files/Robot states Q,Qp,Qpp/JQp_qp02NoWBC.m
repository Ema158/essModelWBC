...
function JQpqp = JQp_qp02NoWBC(robot,JQ)  
...
%[~, ~, JpCoMqp, ~, Jpi_qp] = VelPartialAccCoMs_Frames(robot);
 [~, ~, JpCoMqp, ~, Jpi_qp] = VelAceCoMs_Frames(robot,zeros(24,1));% Podemos utilizar tambien ésta función usando qpp = 0 =)
...
JQpqp(23)= JpCoMqp(1); % The row corresponding to xpp of the CoM
JQpqp(24)= JpCoMqp(2); % The row corresponding to ypp of the CoM
