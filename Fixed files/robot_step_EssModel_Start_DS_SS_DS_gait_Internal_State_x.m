...
function [t,Xt,t_SS,Xt_SS,t_DS,Xt_DS,t_DS_final,Xt_DS_final,gait_parameters_SS,gait_parameters_DS,gait_parameters_DS_final,gait_parameters_gait] = robot_step_EssModel_Start_DS_SS_DS_gait_Internal_State_x(gait_parameters_SS,gait_parameters_DS,gait_parameters_DS_final,gait_parameters_gait,N)
...
yp0 = ypf_DS_final ;
%
xfcyc = gait_parameters.xf;
xmid = gait_parameters.xmid;
% Double Support (SS) Phase
SupportFootX = 0;
SupportFootY = 0;
ZMPd = cell(1,N+1);
...
for j=1:(N)    
    ...
    % IF MOTION IS CYCLIC WE DON'T NEED TO UPGRADE THE COEFFICIENTS
    % The next parameters needs to be updated, in order to rebuild the trajectory
%     gait_parameters.Dx = Dx;
%     gait_parameters.Dy = Dy;
%     gait_parameters.x0cyc = x0;
%     gait_parameters.y0cyc = y0;
%     gait_parameters.xfcyc = xf;
%     gait_parameters.yfcyc = yf;
%     gait_parameters.xpfcyc = xpf;
%     gait_parameters.ypfcyc = ypf;
    % -----------------------------------------   
    % -------------------------------------------------------------
    ...
    PolyCoeff = InitialCoeff_DesiredTrajectories_x(robot,gait_parameters,x0,xfcyc,xmid,xp0);
    ...
    X0 = [x0, y0, xp0, yp0];        
    disp('By using the Essential model of the robot as function of time');
    options = odeset('Events', @PEvents_HDZtime_Internal_State_x,'RelTol', 1.e-7, 'AbsTol', 1.e-9);
    [t{j},Xt{j}] = ode45(@dynam_HZDtime,0:1e-3:1,X0,options);
    ...
    y0 = yt(end);    % Final position in Y of the CoM for step j
    xp0 = xpt(end);  % Final velocity in X of the CoM for step j
    yp0 = ypt(end);  % Final velocity in Y of the CoM for step j
    ...
    fprintf('Initial states: [x0 y0 xp0 yp0] = [%f,%f,%f,%f]\n',xf_DS_final,yf_DS_final,xpf_DS_final,ypf_DS_final);
