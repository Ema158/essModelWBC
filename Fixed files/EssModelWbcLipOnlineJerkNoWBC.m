function [tStart,tStartStep,tStop,tStopStep,t,XtStart,XtStartStep,XtStop,XtStopStep,Xt,SupportFoot] = EssModelWbcLipOnlineJerkNoWBC(X0,gait_parameters_gait,gait_parametersStart,gait_parametersStartStep,gait_parametersStop,gait_parametersStopStep,N,robot,horizon)
global gait_parameters robot xpp
gait_parameters = gait_parametersStart;
S = gait_parameters.S;
D = gait_parameters.D;
Tstart = gait_parameters.T;
% [XtStart] = ode4(@dynam_HZDtimeLipReferenceOnlineJerk,0:1e-3:Tstart,X0);
% tStart = 0:1e-3:Tstart;
[XtStart] = ode4(@dynam_HZDtimeLipReferenceOnlineJerkNoWBC,0:1e-2:Tstart,X0);
...
X_final = XtStart(end,:)';           
% [XtStartStep] = ode4(@dynam_HZDtimeLipReferenceOnlineJerk,0:1e-3:TstartStep,X_final);
% tStartStep = 0:1e-3:TstartStep;
[XtStartStep] = ode4(@dynam_HZDtimeLipReferenceOnlineJerkNoWBC,0:1e-2:TstartStep,X_final);
...
for j=1:N    
    ...
    % ====================================================================
    [t{j},X0,Xt{j}] = robot_step_EssModel_tJerkNoWBC(X_final); % "robot" and "gait_parameters" structures are needed inside
%     for i=1:length(t{j})
...
%     end
    ...
    xpt = Xt{j}(:,3);  % Solution of velocity in X of the CoM for step j
    ypt = Xt{j}(:,4);  % Solution of velocity in Y of the CoM for step j
    % -----------------------------------------------------------------------------------------------------------------
...
% XtStopStep = ode4(@dynam_HZDtimeLipReferenceOnlineJerk,0:1e-3:TstopStep,X_final);
% tStopStep = 0:1e-3:TstopStep;
XtStopStep = ode4(@dynam_HZDtimeLipReferenceOnlineJerkNoWBC,0:1e-2:TstopStep,X_final);
...
% XtStop = ode4(@dynam_HZDtimeLipReferenceOnlineJerk,0:1e-3:Tstop,X_final);
% tStop = 0:1e-3:Tstop;
XtStop = ode4(@dynam_HZDtimeLipReferenceOnlineJerkNoWBC,0:1e-2:Tstop,X_final);
tStop = 0:1e-2:Tstop;
figure(5)
