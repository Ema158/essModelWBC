function [t_gait,Xt_gait,t_SS,Xt_SS,t_DS,Xt_DS,t_DS_final,Xt_DS_final,t_gait_SS,Xt_gait_SS,t_gait_DS,Xt_gait_DS,gait_parameters_SS,gait_parameters_DS,gait_parameters_DS_final,gait_parameters_gait_SS,gait_parameters_gait_DS] = ...
    robot_step_EssModel_Start_DS_SS_DS_gait_SS_DS_t(gait_parameters_SS,gait_parameters_DS,gait_parameters_DS_final,gait_parameters_gait_SS,gait_parameters_gait_DS,N)
...
% axis equal 
%%  Gait phase
gait_parameters = gait_parameters_gait_SS;
T_SS = gait_parameters.T;
T_DS = gait_parameters_gait_DS.T;
D_SS = gait_parameters.D;
S_SS = gait_parameters.S;
g=9.81;
z0_SS = gait_parameters.z_i;
disp('Gait phase')
disp('--------------------------')
% Initial states for SS phase
xf = xf_DS_final;
...
%% Continue...
omega = sqrt(g/z0_SS); % the units are:  sqrt((m/s^2)/m) = 1/s
% ZMPxCoeff = gait_parameters_SS.ZMPxCoeff;
% ZMPyCoeff = gait_parameters_SS.ZMPyCoeff;
t_gait = cell(1,N);
Xt_gait = cell(1,N); 
t_gait_SS = cell(1,N);
Xt_gait_SS = cell(1,N);
...
for j=1:N    
    ...
    [t_gait{j},Xt_gait{j},t_gait_SS{j},Xt_gait_SS{j},t_gait_DS{j},Xt_gait_DS{j},gait_parameters_gait_SS,gait_parameters_gait_DS] = robot_step_EssModel_SSandDSPhases_t(X_final,gait_parameters_gait_SS,gait_parameters_gait_DS); % "robot" and "gait_parameters" structures are needed inside
    ...
    for i=1:samples1
        ZMPdX(i) = polyval(gait_parameters_gait_SS.ZMPxCoeff,t_gait_SS{j}(i));
        zt{j}(i) = polyval(gait_parameters_gait_SS.PolyCoeff.hd1,t_gait_SS{j}(i));
        zpt{j}(i) = polyval(polyder(gait_parameters_gait_SS.PolyCoeff.hd1),t_gait_SS{j}(i));
        if mod(j,2) % If j is impair (for pair steps the evoluition is the opossite direction)
            ZMPdY(i) = polyval(gait_parameters_gait_SS.ZMPyCoeff,t_gait_SS{j}(i));
        else % pair step
            ZMPdY(i) = -polyval(gait_parameters_gait_SS.ZMPyCoeff,t_gait_SS{j}(i));
        end
    end 
    for i=1:samples2
        ZMPdX(samples1 + i) = polyval(gait_parameters_gait_DS.ZMPxCoeff,t_gait_DS{j}(i));
        zt{j}(samples1 + i) = polyval(gait_parameters_gait_DS.PolyCoeff.hd1,t_gait_DS{j}(i));
        zpt{j}(samples1 + i) = polyval(polyder(gait_parameters_gait_DS.PolyCoeff.hd1),t_gait_DS{j}(i));
        if mod(j,2) % If j is impair (for pair steps the evoluition is the opossite direction)
            ZMPdY(samples1 + i) = polyval(gait_parameters_gait_DS.ZMPyCoeff,t_gait_DS{j}(i));
        else % pair step
            ZMPdY(samples1 + i) = -polyval(gait_parameters_gait_DS.ZMPyCoeff,t_gait_DS{j}(i));
        end
    end
    ...
    % --------------------------------------------
    Ex{j} = -(g/(2*z0_SS)).*xt.^2 + (1/2).*xpt.^2; % Orbital energy in X
    Ey{j} = -(g/(2*z0_SS)).*yt.^2 + (1/2).*ypt.^2; % Orbital energy in Y
    ...
    %-------------------------                    
    SupportFootX = SupportFootX + S_SS;
    if mod(j,2) % If j is impair (for pair steps we add distance D)
        SupportFootY = SupportFootY + D_SS;
    else
        SupportFootY = SupportFootY - D_SS; % We come back to the original position
    end
    contB = 1; % Reinitializing the counter of the solver
%     nj=S_SS*j;
%     if mod(j,2)
%     figure(2)
%     plot(Xt_gait{1,j}(:,1) + nj,(Xt_gait{1,j}(:,2) - D_SS)*-1);
%     hold on
%     else
%     figure(2)
%     plot(Xt_gait{1,j}(:,1) + nj,Xt_gait{1,j}(:,2));
%     hold on
%     end
end
