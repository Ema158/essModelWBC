...
function [t,X0,Xt] = robot_step_EssModel_tJerkNoWBC(X_final)
...
[q_end,qp_end,~,~,~] = JointsPosVel_from_CoMPosVel_HZDtimeNoWBC(qf_final,qfp_final,robot,gait_parameters,T);
...
ZeroDynamics_impactTime_testNoWBC(robot.CoM(1),robot.CoM(2),qfp0,robot,gait_parameters);
% ---------------------------------------------------------------------------------
...
% % -----------------------------------------------------------------------------------------------
% % ----------------------------------------------------------------------------------------------------
[Xt] = ode4(@dynam_HZDtimeLipReferenceOnlineJerkNoWBC,0:1e-2:T,X0);
t = 0:1e-2:T;
...
if isempty(OutOfWorkSpace) % If the CoM of the robot is always inside the workspace of the robot....
    ...
    [q,qp,~,~,~] = JointsPosVel_from_CoMPosVel_HZDtimeNoWBC(qf_final,qfp_final,robot,gait_parameters,t(end));
