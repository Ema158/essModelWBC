function q = InvGeometricHZDtimeNoWBC(x,y,robot,gait_parameters,t)
...
Q = [state_vNoWBC(robot);G(1);G(2)];  % Conjunto de posiciones OPERACIONALES reales Q = [h(q)^T, q_f^T]^T
Qdes = [hd; x; y];  % Qd - > Desired controlled variables set and the CoM
...
while nQ>1e-10
    ...
    G = robot.CoM;
    Q = [state_vNoWBC(robot);G(1);G(2)]; % Posiciones operacionales reales
    dQ = Qdes-Q;
    nQ = max(abs(dQ));
    i=i+1;
    if Analyze
