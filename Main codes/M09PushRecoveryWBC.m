...
% Parameters
% ---------------------------------------------------------
% Nao_param = ParamPushRecovery();
Nao_param = ParamOutCoM();
% Nao_param = ParamOutCoMWebots();
% Nao_param = ParamKick();
% ---------------------------------------------------------
...
% GENERAL OPTIONS
% ----------------------------------------------------
% DataName = 'InfoNAO_PushRecovery';
% DataName = 'InfoNAO_Kick';
DataName = 'InfoNAO_OutCoM';
...
% robot.ZMax = robot.Zref + [0.04;0.04]; %Push Recovery
% robot.ZMin = robot.Zref - [0.04;0.04]; %Push Recovery
% robot.ZMax = robot.Zref + [0.02;0.1]; %Standing x
% robot.ZMin = robot.Zref - [0.02;0.1]; %Standing x
...
disp('-------------------------------------------');
X_final = [0.00;0.02;q13f;q14f;q15f;q16f;q17f;q18f;q19f;q20f;Rollf;Pitchf;... %0.021
               xpf;ypf;q13pf;q14pf;q15pf;q16pf;q17pf;q18pf;q19pf;q20pf;Rollpf;Pitchpf;CAMxf;CAMyf];
% [t,Xt,SupportFoot] = EssModelWbcStandingOneFootAccV2(X_final,gait_parameters);
% [t,Xt,SupportFoot] = EssModelWbcStandingOneFootAccWebots(X_final,gait_parameters);
[t,Xt,SupportFoot] = EssModelWbcMpcOutside(X_final,gait_parameters,robot);
%% =====================================
% Plots
% =============
...
axis([min(t) max(t) -inf inf])
xlabel('$t$ [s]','interpreter','Latex','FontSize',FontSize);
ylabel('$Roll(t) [rad]$','interpreter','Latex','FontSize',FontSize);
subplot(2,1,2)  
hold on
plot(t,Xt(:,23),'color','k','LineWidth',LineWidth)
axis([min(t) max(t) -inf inf])
xlabel('$t$ [s]','interpreter','Latex','FontSize',FontSize);
ylabel('$\dot{Roll}(t)$ [rad/s]','interpreter','Latex','FontSize',FontSize);
grid on
%
figure (6)
subplot(2,1,1)  
...
axis([min(t) max(t) -inf inf])
xlabel('$t$ [s]','interpreter','Latex','FontSize',FontSize);
ylabel('$Pitch(t) [rad]$','interpreter','Latex','FontSize',FontSize);
subplot(2,1,2)  
hold on
plot(t,Xt(:,24),'color','k','LineWidth',LineWidth)
axis([min(t) max(t) -inf inf])
xlabel('$t$ [s]','interpreter','Latex','FontSize',FontSize);
ylabel('$\dot{Pitcht}(t)$ [rad/s]','interpreter','Latex','FontSize',FontSize);
grid on
%
figure (7)
subplot(2,1,1)  
