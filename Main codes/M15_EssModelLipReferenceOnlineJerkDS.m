...
%
% Nao_paramSS = ParamQPLIPSSLongStep();
% Nao_paramDS = ParamQPLIPDSLongStep();
% Nao_paramStart = ParamQPLIPStartLongStep();
% Nao_paramStartStep = ParamQPLIPStartSSLongStep();
% Nao_paramStop = ParamQPLIPStopLongStep();
% Nao_paramStopStep = ParamQPLIPStopSSLongStep();
% ---------------------------------------------------------
...
robot = genebotV3Jerk(gait_parametersSS,horizon);
horizon = round(T/gait_parametersSS.Tmuestreo);
[robot.Zref,robot.tRef] = ZMPReferenceV2(N,horizon,S,D);
[robot.ZMax,robot.ZMin] = ZMPConstraints(N,horizon,S,D);
...
[tStart,tStartStep,tStop,tStopStep,tSS,tDS,XtStart,XtStartStep,XtStop,XtStopStep,XtSS,XtDS,SupportFoot] = EssModelWbcLipOnlineJerkDS(X_final,gait_parametersSS,gait_parametersDS,gait_parametersStart,gait_parametersStartStep,gait_parametersStop,gait_parametersStopStep,N,horizon);
%% =====================================
% Plots
% =============
...
for j=1:(N+4)
    ...
    if j~=1 && j~=2 && j~=N+3 && j~=N+4
        ...
        samplesSS = length(tSS{j-2});
        ZMPxCoeffSS = gait_parametersSS.ZMPxCoeff;
        ZMPyCoeffSS = gait_parametersSS.ZMPyCoeff;
        ZMPdXSS = zeros(1,samplesSS);
        ZMPdYSS = zeros(1,samplesSS);
        for i=1:samplesSS
            ZMPdXSS(i) = polyval(ZMPxCoeffSS,tSS{j-2}(i));
            ztSS{j}(i) = polyval(gait_parametersSS.PolyCoeff.hd1,tSS{j-2}(i));
            zptSS{j}(i) = polyval(polyder(gait_parametersSS.PolyCoeff.hd1),tSS{j-2}(i));
            if mod(j,2) % If j is impair (for pair steps the evoluition is the opossite direction)
                ZMPdYSS(i) = polyval(ZMPyCoeffSS,tSS{j-2}(i));
            else % pair step
                ZMPdYSS(i) = -polyval(ZMPyCoeffSS,tSS{j-2}(i));
            end
        end
        ZMPdSS{j}(1,:) = SupportFoot{j}(1) + ZMPdXSS;
        ZMPdSS{j}(2,:) = SupportFoot{j}(2) + ZMPdYSS;
        %
        samplesDS = length(tDS{j-2});
        ZMPxCoeffDS = gait_parametersDS.ZMPxCoeff;
        ZMPyCoeffDS = gait_parametersDS.ZMPyCoeff;
        ZMPdXDS = zeros(1,samplesDS);
        ZMPdYDS = zeros(1,samplesDS);
        for i=1:samplesDS
            ZMPdXDS(i) = polyval(ZMPxCoeffDS,tDS{j-2}(i));
            ztDS{j}(i) = polyval(gait_parametersDS.PolyCoeff.hd1,tDS{j-2}(i));
            zptDS{j}(i) = polyval(polyder(gait_parametersDS.PolyCoeff.hd1),tDS{j-2}(i));
            if mod(j,2) % If j is impair (for pair steps the evoluition is the opossite direction)
                ZMPdYDS(i) = polyval(ZMPyCoeffDS,tDS{j-2}(i));
            else % pair step
                ZMPdYDS(i) = -polyval(ZMPyCoeffDS,tDS{j-2}(i));
            end
        end
        ZMPdDS{j}(1,:) = SupportFoot{j}(1) + ZMPdXDS;
        ZMPdDS{j}(2,:) = SupportFoot{j}(2) + ZMPdYDS;
        zt{j} = [ztSS{j},ztDS{j}(2:end)];
        zpt{j} = [zptSS{j},zptDS{j}(2:end)];
    end
    if j==N+3
