...
% Options
% ----------------------------------------------- 
% DataName = 'InfoNAO_DS'; % File produced in "M04d_...m" CHANGE OptionContVar = 1;
DataName = 'InfoNAO_DSAcc'; % File produced in "M04d_...m" CHANGE OptionContVar = 1;
...
%     X0 = Xt{j}(1,:); % Initial condition of the CoM
    ...
    if j~=1 && j~=Nstep+2
        [qtSS{j},qDtSS{j},qDDtSS{j}] = joints_fromCoMpos_Vel_HZDtime(robot,XtSS{j-1},gait_parametersSS,tSS{j-1});
        [qtDS{j},qDtDS{j},qDDtDS{j}] = joints_fromCoMpos_Vel_HZDtime(robot,XtDS{j-1},gait_parametersDS,tDS{j-1});
...
%             qtSwap = [qt{j}(12,:); -qt{j}([11,10,9],:); qt{j}([8,7,6,5],:); -qt{j}([4,3,2],:); qt{j}(1,:); qt{j}(13,:);  qt{j}(14:24,:)];
%             qDtSwap = [qDt{j}(12,:); -qDt{j}([11,10,9],:); qDt{j}([8,7,6,5],:); -qDt{j}([4,3,2],:); qDt{j}(1,:); -qDt{j}(13,:);  qDt{j}(14:24,:)];
%             qDDtSwap = [qDDt{j}(12,:); -qDDt{j}([11,10,9],:); qDDt{j}([8,7,6,5],:); -qDDt{j}([4,3,2],:); qDDt{j}(1,:); -qDDt{j}(13,:);  qDDt{j}(14:24,:)];
            
            qtSwap = [qt{j}(12,:); -qt{j}([11,10,9],:); qt{j}(8,:); -qt{j}([7,6],:); qt{j}(5,:) ;-qt{j}([4,3,2],:); qt{j}(1,:);...
                      qt{j}(18,:);  -qt{j}(19:21,:); qt{j}(17,:); qt{j}(13,:); -qt{j}(14:16,:); qt{j}(22,:); -qt{j}(23,:); qt{j}(24,:)];
            qDtSwap = [qDt{j}(12,:); -qDt{j}([11,10,9],:); qDt{j}(8,:); -qDt{j}([7,6],:); qDt{j}(5,:) ;-qDt{j}([4,3,2],:); qDt{j}(1,:);...
                      qDt{j}(18,:);  -qDt{j}(19:21,:); qDt{j}(17,:); qDt{j}(13,:); -qDt{j}(14:16,:); qDt{j}(22,:); -qDt{j}(23,:); qDt{j}(24,:)];
            qDDtSwap = [qDDt{j}(12,:); -qDDt{j}([11,10,9],:); qDDt{j}(8,:); -qDDt{j}([7,6],:); qDDt{j}(5,:) ;-qDDt{j}([4,3,2],:); qDDt{j}(1,:);...
                      qDDt{j}(18,:);  -qDDt{j}(19:21,:); qDDt{j}(17,:); qDDt{j}(13,:); -qDDt{j}(14:16,:); qDDt{j}(22,:); -qDDt{j}(23,:); qDDt{j}(24,:)];
...
pxMax = Solution.pMaxX;
pyMax = Solution.pMaxY;
pxMin = Solution.pMinX;
pyMin = Solution.pMinY;
ZMPMax = [pxMax;pyMax];
ZMPMin = [pxMin;pyMin];
% [Zref,tRef] = ZMPReferenceV3OneStep(100,0,S,0,D);
FootCenter = [0,0];
...
ZMPfootprints(ZMP{1},FootCenter,width,length2Ankle,length2toes,'k')
...
figure(33)
plot(tStartAll,XtAllStart(:,25))
hold on
for j=1:Nstep
    if mod(j,2)
        plot(tStartAll(end)+t{1}+t{1}(end)*(j-1),-Xt{j}(:,25))
    else
        plot(tStartAll(end)+t{1}+t{1}(end)*(j-1),Xt{j}(:,25))
    end
end
if mod(Nstep,2)
    plot(tStartAll(end)+t{1}(end)*(Nstep)+tStopAll,XtStopAll(:,25))
else
    plot(tStartAll(end)+t{1}(end)*(Nstep)+tStopAll,-XtStopAll(:,25))
end
title('Angular Momentum in x');
figure(34)
plot(tStartAll,XtAllStart(:,26))
hold on
for j=1:Nstep
    plot(tStartAll(end)+t{1}+t{1}(end)*(j-1),Xt{j}(:,26))
end
plot(tStartAll(end)+t{1}(end)*(Nstep)+tStopAll,XtStopAll(:,26))
