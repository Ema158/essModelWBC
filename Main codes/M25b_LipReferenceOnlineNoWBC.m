...
% Options
% ----------------------------------------------- 
DataName = 'InfoNAO_Param_t_caminadoNoWBC'; % File produced in "M04d_...m" CHANGE OptionContVar = 1;
...
%     X0 = Xt{j}(1,:); % Initial condition of the CoM
    ...
    if j~=1 && j~=Nstep+2
        [qt{j},qDt{j},qDDt{j}] = joints_fromCoMpos_Vel_HZDtimeNoWBC(robot,Xt{j-1},gait_parameters_gait,t{j-1});
...
for j=1:(Nstep+2)
    ...
    for i=1:samples
        ...
        [Q{j}(:,i), Qp{j}(:,i)] = current_statesNoWBC(robot);
    end
end
...
figure(31)
plot([tStart,tStart(end)+tStartStep],ZMP{1}(2,:))
hold on
for i=1:Nstep
    if mod(i,2)==0
        plot(tStart(end)+tStartStep(end)+t{i}+0.5*(i-1),ZMP{i+1}(2,:))
    else
        plot(tStart(end)+tStartStep(end)+t{i}+0.5*(i-1),-ZMP{i+1}(2,:)+D)
    end
end
if mod(Nstep,2)==1
    plot([tStart(end)+tStartStep(end)+0.5*(Nstep)+tStop,tStart(end)+tStartStep(end)+0.5*(Nstep)+tStop(end)+tStopStep],ZMP{Nstep+2}(2,:))
else
    plot([tStart(end)+tStartStep(end)+0.5*(Nstep)+tStop,tStart(end)+tStartStep(end)+0.5*(Nstep)+tStop(end)+tStopStep],-ZMP{Nstep+2}(2,:)+D)
end
%% End of the code
% ----------------------------------------------------------------------------------------------
