...
% GENERAL OPTIONS
% ----------------------------------------------------
DataName = 'InfoNAO_Param_t_caminadoNoWBC';
...
[tStart,tStartStep,tStop,tStopStep,t,XtStart,XtStartStep,XtStop,XtStopStep,Xt,SupportFoot] = EssModelWbcLipOnlineJerkNoWBC(X_final,gait_parameters_gait,gait_parametersStart,gait_parametersStartStep,gait_parametersStop,gait_parametersStopStep,N,robot,horizon);
%% =====================================
% Plots
% =============
...
for j=1:(N+4)
    ...
    if j~=1 && j~=2 && j~=N+3 && j~=N+4
        ...
        if mod(j,2)==false % If j is impair (for pair steps we add distance D and invert direction)    
            ...
            ypt = Xt{j-2}(:,4)';                       % global velocity yp(t) for step "j" 
        else
            yt = SupportFoot{j}(2) - Xt{j-2}(:,2)';  % global position of y(t) for step "j"
            ypt = -Xt{j-2}(:,4)';                       % global velocity yp(t) for step "j" 
        end
        xpt = Xt{j-2}(:,3)';                       % xp(t) for step "j"     
