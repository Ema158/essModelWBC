function [Zref,tRef] = ZMPReferenceTrajectory(N)
    Zref = zeros(2,7*N);
    ...
    %
    Zref(1,7*N+1:8*N) = linspace(0.02,0.00,N);
    Zref(2,7*N+1:8*N) = 0.05*ones(1,N);
%     Zref(1,1:3*N) = 0.02*ones(1,3*N);
%     Zref(2,1:3*N) = 0.05*ones(1,3*N);
%     %
%     Zref(1,3*N+1:7*N) = 0.02*ones(1,4*N);
%     Zref(2,3*N+1:7*N) = 0.1*ones(1,4*N);
    %
