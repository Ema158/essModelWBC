function [Zref,tRef] = ZMPReferenceTrajectoryV2Prueba(N,muestrasTotal)
%     Zref = zeros(2,muestrasTotal+2*N);
%     Zref(2,:) = 0.05*ones(1,muestrasTotal+2*N);
    Zref = 0.04*ones(2,muestrasTotal+2*N);
    Zref(2,:) = 0.1*ones(1,muestrasTotal+2*N);
    %
    Zref(1,1:N) = 0.0*ones(1,N);
    Zref(2,1:N) = 0.05*ones(1,N);
    %
    Zref(1,N+1:2*N) = 0.00*ones(1,N);
    Zref(2,N+1:2*N) = 0.0*ones(1,N);
    %
    Zref(1,2*N+1:3*N) = 0.02*ones(1,N);
    Zref(2,2*N+1:3*N) = 0.1*ones(1,N);
%     %
    Zref(1,3*N+1:4*N) = 0.04*ones(1,N);
    Zref(2,3*N+1:4*N) = 0.0*ones(1,N);
%     %
