function   Nao_param = ParamEsencial_z_t_StartingSS_circV3()
...
Yaw_ffoot_f = deg2rad(20); % final orientation of the free foot w.r.t. the support frame (right foot)
Yaw_ffoot_inc = Yaw_ffoot_f-Yaw_ffoot_i; % angle displacement of the free foot
ffoot_pos_f_0 =  [S;D-wide_error];
RotZ_01 = [cos(Yaw_ffoot_inc), -sin(Yaw_ffoot_inc); sin(Yaw_ffoot_inc), cos(Yaw_ffoot_inc)];
ffoot_pos_f_1 = RotZ_01*ffoot_pos_f_0;
Step_length_1 = ffoot_pos_f_1(1) + S;
wide_error_1 = ffoot_pos_f_1(2) - D;
...
z0 = 0.255;       % hight of the CoM
g = 9.81;        % acceleration of gravity
% --------------
% Parte superior del cuerpo
RShoulder_Pitch_i = 0.3;%q13 Q11
RShoulder_Pitch_f = -0.2;%q13 Q11
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
LShoulder_Pitch_i = 0.3;%q17 Q15
LShoulder_Pitch_f = 1.2;%q17 Q15
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
RShoulder_Yaw_i = -0.8;%q14 Q12
RShoulder_Yaw_f = -0.8;%q14 Q12
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
LShoulder_Yaw_i = 0.8;%q18 Q16
LShoulder_Yaw_f = 0.8;%q18 Q16
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
RElbow_Yaw_i = 0.4;%q16 Q14
RElbow_Yaw_f = 0.3;%q16 Q14
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
LElbow_Yaw_i = -0.9;%q20 Q18
LElbow_Yaw_f = -1.4;%q20 Q18
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
%Tiempos
T_midFoot=T/2;
T_midCoMz=T/2;
...
%% Desired ZMP
% =========================================
% THIS initial values are gotten from the one obtained with 9 parameters (but we could chose any other)
% Proposed ZMP in initial SS:
% ZMPx0 = 0.000166205071429;  %Param70b
% ZMPxf = -0.007285213853954;  %Param70b
% ZMPy0 =  0.023600840866919;   %Param70b
% ZMPyi = 0;                     %Param70b
% ZMPyf = 0.001304735744782; %Param70b
%
ZMPx0 = -0.000034929784661;  %Param70b2
ZMPxf = -0.006906159727000;  %Param70b2
ZMPy0 =  0.024652800274080;   %Param70b2
ZMPyi = 0;                     %Param70b2
ZMPyf = -0.005742519771140; %Param70b2
% ---------------------------------------------------------------------
...
gait_parameters.x_ffoot_f = ffoot_pos_f_1(1);   % Distance traveled in X by the free foot to reach the final position in X
gait_parameters.y_ffoot_i = D;       % Initial step position in Y
gait_parameters.y_ffoot_f = ffoot_pos_f_1(2);      % Distance traveled in Y by the free foot to reach the final position in Y
...
gait_parameters.Yaw_ffoot_f = Yaw_ffoot_f; % Rotation in Z to be performed by the free foot to reach its final orientation in Z
% Hip yaw
...
%Rcyc = [-1.384055e-03,2.604667e-04,7.845084e-02,2.369689e-01]; %Valores encontrados con el programa M40
%Rcyc = [2.920938e-02,3.276052e-04,2.804558e-02,2.397634e-01];zmp 0.01-0.07
%Rcyc = [1.430227e-02,2.542518e-03,6.719091e-02,2.278441e-01];
%Rcyc = [1.427724e-02,4.727710e-03,6.708265e-02,2.405883e-01];
%Rcyc = [4.107068e-03,2.550897e-03,9.388333e-02,2.285039e-01];
%Rcyc = [4.105139e-03,3.697734e-04,9.399808e-02,2.156963e-01];
%Rcyc = [3.896062e-03,4.592938e-03,9.537590e-02,2.157402e-01]; %Param00_ZMPx_var
%Rcyc = [3.882306e-03,6.824647e-03,9.517380e-02,2.287577e-01]; %Param00_ZMPx_var2
%Rcyc = [3.847286e-03,1.525276e-04,9.577531e-02,2.158708e-01];
% Rcyc = [3.158604e-03,1.750177e-04,1.224229e-01,2.156317e-01];
% Rcyc = [3.073614e-03,2.270968e-03,1.218660e-01,2.124650e-01]; % Param_var4 ZMPy 0 - 0.1 z=0.26
% Rcyc = [1.199074e-02,1.197941e-02,9.848031e-02,1.657027e-01];
Rcyc = [0.013234087227064,0.014903196111182,0.085585051253527,0.169639636131328];
