function   Nao_param = ParamKickCAM()
...
Pitch_ffoot_inc=0;
% Parámetros patada
l = 0.25; %Largo de la pierna durante la patada
theta = deg2rad(10); %ángulo de inclinación de la pierna
%% Desired CAM
% ---------------------------------------------------------------------
...
CAMxIni =  0;      % Local position of the desired ZMP in X for each step
CAMxEnd =  0;
CAMyIni =  0;      % Local position of the desired ZMP in Y for each step
CAMyMid = -0.15; %-0.15
CAMyEnd =  0;
Pos = [T1 CAMxIni;       
       T2 CAMxEnd];
Vel = [T1 0
       T2 0];
Acc = [];
CAMxCoeff = findPolyCoeff(Pos,Vel,Acc);
CAMpxCoeff = polyder(CAMxCoeff)';
% Pos = [T1 CAMyIni;
%        T/2 CAMyMid; 
%        T2 CAMyEnd];     
Pos = [T1 CAMyIni;
...
Vel = [T1 0;
     0.32 0;
     T2 0];
CAMyCoeff = findPolyCoeff(Pos,Vel,Acc);
CAMpyCoeff = polyder(CAMyCoeff)';
% %% Desired CAMp
% % ---------------------------------------------------------------------
% T1 = 0; 
% T2 = T;
...
%        T2 0];
% Acc = [];
% CAMpxCoeff = findPolyCoeff(Pos,Vel,Acc);
% Pos = [T1 CAMpyIni;
%        T2 CAMpyEnd];     
% CAMpyCoeff = findPolyCoeff(Pos,Vel,Acc);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
gait_parameters.x_ffoot_i = -l*sin(theta);      % Initial step position in X
gait_parameters.x_ffoot_f = l*sin(theta);   % Distance traveled in X by the free foot to reach the final position in X
...
gait_parameters.z_ffoot_i = z0 - l*cos(theta);       % Initial step position in Z
gait_parameters.z_ffoot_f = z0 - l*cos(theta);       % Final step position in Z
...
gait_parameters.CAMxIni = CAMxIni;   % Initial desired position in X
gait_parameters.CAMyIni = CAMyIni;   % Initial desired position in Y
gait_parameters.CAMxEnd = CAMxEnd;   % Final desired position in X
gait_parameters.CAMyEnd = CAMyEnd;   % Final desired position in Y
gait_parameters.CAMxCoeff = CAMxCoeff; % Coefficients for the polynomial trayectory of the ZMP in X
gait_parameters.CAMyCoeff = CAMyCoeff; % Coefficients for the polynomial trayectory of the ZMP in Y
% CAMp
% gait_parameters.CAMpxIni = CAMpxIni;   % Initial desired position in X
% gait_parameters.CAMpyIni = CAMpyIni;   % Initial desired position in Y
% gait_parameters.CAMpxEnd = CAMpxEnd;   % Final desired position in X
% gait_parameters.CAMpyEnd = CAMpyEnd;   % Final desired position in Y
gait_parameters.CAMpxCoeff = CAMpxCoeff; % Coefficients for the polynomial trayectory of the ZMP in X
gait_parameters.CAMpyCoeff = CAMpyCoeff; % Coefficients for the polynomial trayectory of the ZMP in Y
...
%
%
Q(3:10,3:10) = 0.0001*eye(8,8); % $Push Recovery PD 1-7
Q(11,11) = 0.006; % $Push Recovery PD 1-7
Q(12,12) = 0.006; % $Push Recovery PD 1-7
%
