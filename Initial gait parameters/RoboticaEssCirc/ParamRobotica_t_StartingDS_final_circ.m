function   Nao_param = ParamRobotica_t_StartingDS_final_circ()
% based on "Param13b_t_SS_NOimpact_ZMPvar_HipTorsoArmsNeck" 

% Parameters for a walking gait base on TIME:
% In here only the evolution of the ZMP change
...
Yaw_ffoot_i = deg2rad(0); % Initial orientation of the free foot w.r.t. the support frame (right foot) %Estos valores no se mandan!!!!!
Yaw_ffoot_f = deg2rad(20); % final orientation of the free foot w.r.t. the support frame (right foot) %Estos valores no se mandan!!!!!
...
RShoulder_Pitch_i = 1.4;%q13 Q11
RShoulder_Pitch_f = 1.4;%q13 Q11
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
LShoulder_Pitch_i = 1.8;%q17 Q15
...
% ZMPxIni = -0.014494311511032; %0.25
% ZMPxEnd = S-0.005;  
% ZMPyIni = -0.004966314516325; %0.25  q6 q7
% ZMPyMid = 0.0425; %Puede usarse o no
% ZMPyEnd = D;  
%
% ZMPxIni = -0.014825818970788; %0.25
% ZMPxEnd = S-0.005;  
% ZMPyIni = -0.004678736484121; %0.25
% ZMPyMid = 0.0425; %Puede usarse o no
% ZMPyEnd = D; 
%
ZMPxIni = -0.001077612285084; %0.25
ZMPxEnd = ffoot_pos_f_1(1)-0.005;  
ZMPyIni = 0.017630314368767; %0.25
ZMPyMid = 0.0425; %Puede usarse o no
ZMPyEnd = ffoot_pos_f_1(2); 
Pos = [T1 ZMPxIni; 
%        T/2 ZMPxMid;
       T2 ZMPxEnd];
Vel = [];
...
gait_parameters.x_ffoot_i = ffoot_pos_f_1(1);      % Initial step position in X
gait_parameters.x_ffoot_f = ffoot_pos_f_1(1);   % Distance traveled in X by the free foot to reach the final position in X
gait_parameters.y_ffoot_i = ffoot_pos_f_1(2);       % Initial step position in Y
...
gait_parameters.Yaw_ffoot_i = Yaw_ffoot_f;     % Initial rotation in Z of the free foot
...
gait_parameters.ZMPyMid = ZMPyMid;   % Desired position in Y in T/2
...
% Rcyc = [0.012054379338581,0.027637007192283,0.060011230470480,0.119097942956146]; %ZMP 0_0 h_f= 0.02
% Rcyc = [-0.013667002010976,0.006182570707961,0.163118041254171,0.265973214454775]; % q6,q7   10 grados 
Rcyc = [-0.020320749960810,0.017529581068308,0.099046785474790,0.247550647278659];
